...
def evaluate_phonebook(model, tokenizer, data_point: Dict[str, Any], prompt_template: str) -> Dict[str, Any]:
    """
    Evaluates a single sample from the PhoneBook dataset using the given prompt template.
    """
    device = model.device
    ...
    prompt = apply_qwen_chat_template(tokenizer, user_message)
    ...
    return {
        "id": data_point['id'], "dataset": "phonebook", "metadata": data_point['metadata'],
