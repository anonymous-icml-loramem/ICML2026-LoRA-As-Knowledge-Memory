...
def evaluate_babilong(model, tokenizer, data_point: Dict[str, Any]) -> Dict[str, Any]:
    """
    Evaluates a single sample from the Babilong dataset generatively and returns a detailed log.
    ...
    # Simplified application of the prompt format from the Babilong paper
    prompt = (
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_new_tokens=50, # Limit to 50 tokens since answers are usually short
    ...
    # Normalize and compare for Exact Match
    ...
    # Create log to be recorded in raw_predictions.jsonl
    raw_log = {
        "id": data_point['id'],
...
def evaluate_quality(model, tokenizer, data_point: Dict[str, Any]) -> Dict[str, Any]:
    """
    Evaluates a single sample from the QUALITY dataset in a multiple-choice format and returns a detailed log.
    Metric: Accuracy
    """
    device = model.device
    context = data_point['context']
    ...
    with torch.no_grad():
        for i, option_text in enumerate(options):
            # Generate full input for each option (e.g., <prompt> A. {option_text})
            ...
            # Tokenize the answer part separately to define loss calculation range
            answer_tokens = tokenizer(answer_part, add_special_tokens=False).input_ids
            answer_len = len(answer_tokens)
            
            # Extract logits corresponding to the answer from the total logits
